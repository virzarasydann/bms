...
class KategoriPemasukanController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $opd = KategoriPenerimaan::orderBy('id', 'asc');

            return DataTables::of($opd)
                ->addIndexColumn()
               ->addColumn('action', function ($row) use ($permissions): string {
                    $editUrl = route('kategoriPemasukan.edit', $row->id);
                    $deleteUrl = route('kategoriPemasukan.destroy', $row->id);
        ...
        return view('admin.kategori_pemasukan.index', compact('permissions'));
    }

    public function edit($id)
    {
        $list = KategoriPenerimaan::findOrFail($id);
    ...
    {
        $data = KategoriPenerimaan::findOrFail($id);
